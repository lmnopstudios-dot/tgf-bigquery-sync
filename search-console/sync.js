...
export const STAGE_INSERT_BATCH_SIZE=500;
const safe=v=>{if(!/^[A-Za-z0-9_-]+$/.test(v))throw new Error('Invalid BigQuery identifier');return v};

function operationDiagnostics(error,{operation,table=null,sourceProperty=null,startDate,endDate,accumulatedRows=null,batchSize=null,batchOffset=null}){
  const stackOverflow=(error instanceof RangeError||error?.name==='RangeError')&&/maximum call stack size exceeded/i.test(String(error.message));
  const rowLimit=error?.name==='SearchConsoleRowLimitError';
  return {operation,logical_target_table:table,source_property:sourceProperty,requested_range:{start_date:startDate,end_date:endDate},accumulated_row_count:accumulatedRows,batch_size:batchSize,batch_offset:batchOffset,error_class:error?.name||error?.constructor?.name||'Error',message:stackOverflow?'Maximum call stack size exceeded':rowLimit?'Search Console response reached bounded row limit':'Operation failed; message redacted'};
}
export class SearchConsoleOperationError extends Error{
  constructor(diagnostics,{cause}={}){super(JSON.stringify(diagnostics));this.name='SearchConsoleOperationError';this.diagnostics=diagnostics;this.cause=cause}
}
function contextualize(error,context){
  if(error instanceof SearchConsoleOperationError||error instanceof StageInsertError||error instanceof StageSchemaError)return error;
  return new SearchConsoleOperationError(operationDiagnostics(error,context),{cause:error});
}
...
export async function insertStageRows({bigquery,project,dataset,stage,table,sourceProperty,rows,startDate=null,endDate=null,batchSize=STAGE_INSERT_BATCH_SIZE}){
  const target=bigquery.dataset(dataset,{projectId:project}).table(stage);
  for(let offset=0;offset<rows.length;offset+=batchSize){
    const batch=rows.slice(offset,offset+batchSize);
    try{await target.insert(batch)}catch(error){
      if(error?.name==='PartialFailureError')throw new StageInsertError({...partialFailureDiagnostics(error,{table,sourceProperty,batchSize:batch.length,rows:batch}),requested_range:{start_date:startDate,end_date:endDate},accumulated_row_count:offset,batch_offset:offset},{cause:error});
      throw contextualize(error,{operation:'bigquery_stage_insert',table,sourceProperty,startDate,endDate,accumulatedRows:offset,batchSize:batch.length,batchOffset:offset});
    }
  }
  return rows.length;
}
...
export async function fetchRows(client,siteUrl,startDate,endDate,dimensions,{maxRows=500000,pageSize=25000,table=null}={}){const out=[];for(let offset=0;offset<maxRows;offset+=pageSize){let response;try{response=await retry(()=>client.searchanalytics.query({siteUrl,requestBody:{startDate,endDate,dimensions,dataState:'final',aggregationType:'auto',rowLimit:Math.min(pageSize,maxRows-offset),startRow:offset}}))}catch(error){throw contextualize(error,{operation:'search_console_collection',table,sourceProperty:siteUrl,startDate,endDate,accumulatedRows:out.length})}const batch=response?.data?.rows??response?.rows??[];for(const row of batch)out.push(row);if(batch.length<Math.min(pageSize,maxRows-offset))return out}const error=new Error('bounded row cap reached');error.name='SearchConsoleRowLimitError';throw contextualize(error,{operation:'search_console_collection',table,sourceProperty:siteUrl,startDate,endDate,accumulatedRows:out.length})}
...
function appendConverted(target,raw,dimensions,meta,syncedAt){for(const r of raw){const values=Object.fromEntries(dimensions.map((d,i)=>[d,r.keys?.[i]??''])),base={...meta,date:values.date,...metricRow(r),coverage_status:'available',synced_at:syncedAt};if(values.query!==undefined)base.query=values.query;if(values.page!==undefined)Object.assign(base,{page:values.page,...normalizePage(values.page)});if(values.device!==undefined)base.device=values.device;if(values.country!==undefined)base.country=values.country;target.push(base)}}
export async function collect({client,startDate,endDate,maxRows=500000,syncedAt=new Date().toISOString()}){const data=Object.fromEntries(TABLES.map(t=>[t,[]]));for(const meta of PROPERTIES){const specs={daily:['date'],queries:['date','query'],pages:['date','page'],device_country:['date','device','country']};for(const [name,dims] of Object.entries(specs)){const raw=await fetchRows(client,meta.source_property,startDate,endDate,dims,{maxRows,table:name});try{appendConverted(data[name],raw,dims,meta,syncedAt)}catch(error){throw contextualize(error,{operation:'search_console_normalization',table:name,sourceProperty:meta.source_property,startDate,endDate,accumulatedRows:data[name].length})}}}for(const name of TABLES)try{validateRows(name,data[name])}catch(error){throw contextualize(error,{operation:'search_console_validation',table:name,startDate,endDate,accumulatedRows:data[name].length})}try{data.canonical_daily=canonicalDaily(data.daily,startDate,endDate,syncedAt)}catch(error){throw contextualize(error,{operation:'canonical_construction',table:'canonical_daily',startDate,endDate,accumulatedRows:data.daily.length})}return data}
...
export async function promote({bigquery,project,dataset,data,startDate,endDate,invocationId=randomUUID().replaceAll('-','_')}){
  ...
  try {
    ...
    for(const property of PROPERTIES) for(const table of TABLES){
      ...
      await insertStageRows({bigquery,project,dataset,stage,table,sourceProperty:property.source_property,rows,startDate,endDate});
    ...
    await insertStageRows({bigquery,project,dataset,stage:canonicalStage,table:'canonical_daily',sourceProperty:null,rows:data.canonical_daily,startDate,endDate});
