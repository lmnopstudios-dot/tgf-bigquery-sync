  ...
  return {
    ...
    moments: `WITH m AS (SELECT * FROM ${window(m)})
      SELECT
       ...
       STRUCT(COUNTIF(utm_source IS NOT NULL OR utm_medium IS NOT NULL OR utm_campaign IS NOT NULL OR utm_content IS NOT NULL OR utm_term IS NOT NULL) AS with_any_utm, COUNT(*) total) all_moments_utm
      FROM m`,
    coverage: `WITH a AS (SELECT * FROM ${window(a)}) SELECT
      STRUCT(COUNTIF(first_visit_utm_source IS NOT NULL OR first_visit_utm_medium IS NOT NULL OR first_visit_utm_campaign IS NOT NULL OR first_visit_utm_content IS NOT NULL OR first_visit_utm_term IS NOT NULL) AS with_any_utm, COUNTIF(first_visit_id IS NOT NULL) total_visits) first_visits,
      STRUCT(COUNTIF(last_visit_utm_source IS NOT NULL OR last_visit_utm_medium IS NOT NULL OR last_visit_utm_campaign IS NOT NULL OR last_visit_utm_content IS NOT NULL OR last_visit_utm_term IS NOT NULL) AS with_any_utm, COUNTIF(last_visit_id IS NOT NULL) total_visits) last_visits,
      STRUCT(COUNTIF(days_to_conversion IS NULL) null_count, MIN(days_to_conversion) min, MAX(days_to_conversion) max, AVG(days_to_conversion) average,
        COUNTIF(days_to_conversion=0) same_day, COUNTIF(days_to_conversion BETWEEN 1 AND 7) days_1_7, COUNTIF(days_to_conversion BETWEEN 8 AND 30) days_8_30, COUNTIF(days_to_conversion BETWEEN 31 AND 90) days_31_90, COUNTIF(days_to_conversion>90) days_over_90) conversion_lag
      FROM a`,
