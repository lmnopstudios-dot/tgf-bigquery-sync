...
export async function checkOracleJobReadiness({bigquery,project,dataset=ORACLE_JOB_DEFAULTS.dataset,table=ORACLE_JOB_DEFAULTS.table,expectedLocation=ORACLE_JOB_DEFAULTS.location}){
  ...
  await run('parameter_binding',async()=>{await bigquery.createQueryJob({query:`INSERT INTO \`${project}.${dataset}.${table}\` (job_id,owner_key,request_id,status,payload_json,created_at,updated_at,attempts,cancel_requested) VALUES (@job_id,@owner_key,@request_id,'queued',PARSE_JSON(@payload_json),@created_at,@created_at,0,FALSE)`,params:{job_id:'readiness-probe',owner_key:'readiness-probe',request_id:'oracle-smoke-readiness',payload_json:'{"synthetic":true}',created_at:new Date(0)},types:{job_id:'STRING',owner_key:'STRING',request_id:'STRING',payload_json:'STRING',created_at:'TIMESTAMP'},location:result.actual_location,dryRun:true,useLegacySql:false,labels:{component:'oracle_jobs',operation:'readiness'}});return {dry_run:true,location:result.actual_location};});
