...
export const PRODUCT_MAPPING_OPERATION_TABLE = 'product_mapping_operations';
...
export const PRODUCT_MAPPING_OPERATION_SCHEMA=Object.freeze(['event_id:STRING','operation_id:STRING','row_index:INTEGER','row_request_id:STRING','action:STRING','source_ref:STRING','shopify_parent_ref:STRING','status:STRING','decision_event_id:STRING','error:STRING','reviewed_by:STRING','recorded_at:TIMESTAMP']);
export const PRODUCT_MAPPING_MAX_BULK_SIZE = 8;
...
export function createProductMappingService({ bigquery, project, dataset = PRODUCT_MAPPING_DATASET }) {
  ...
  const operationTable = `${project}.${dataset}.${PRODUCT_MAPPING_OPERATION_TABLE}`;
  ...
  const setup = async () => atGovernanceStage('ensure_schema','product_mapping_ensure_table',async() => {
    ...
    const operations=ds.table(PRODUCT_MAPPING_OPERATION_TABLE);const [operationsFound]=await operations.exists();if(!operationsFound)await operations.create({schema:{fields:PRODUCT_MAPPING_OPERATION_SCHEMA.map(spec=>{const [name,type]=spec.split(':');return{name,type};})},timePartitioning:{type:'DAY',field:'recorded_at'},labels:{component:'product_mapping_operation'}});
  });
...
SELECT * EXCEPT(exact_sku,local_title_count,title_sources,local_sku_count,sku_sources),IF((exact_sku IS NOT NULL AND local_sku_count=1 AND sku_sources>1) OR (normalized_title!='' AND local_title_count=1 AND title_sources>1),'resolved','source_specific') mapping_status FROM keyed`,useLegacySql:false,maximumBytesBilled:'10000000000',labels:{component:'product_mapping',operation:'load_products'}}); return rows; });
  ...
  const operationEvents=async operationId=>atGovernanceStage('load_current_decision','product_mapping_operation_status',async()=>{const [rows]=await bigquery.query({query:`SELECT * FROM \`${operationTable}\` WHERE operation_id=@operation_id ORDER BY row_index,recorded_at,event_id`,params:{operation_id:operationId},types:{operation_id:'STRING'},useLegacySql:false});return rows;});
  const appendOperationEvent=async row=>atGovernanceStage('insert_decision','product_mapping_operation_append',async()=>{const event={...row,event_id:stableId(row.operation_id,`operation:${row.row_index}:${row.status}`),recorded_at:new Date().toISOString()};await bigquery.query({query:`MERGE \`${operationTable}\` target USING (SELECT JSON_VALUE(@payload,'$.event_id') event_id,JSON_VALUE(@payload,'$.operation_id') operation_id,SAFE_CAST(JSON_VALUE(@payload,'$.row_index') AS INT64) row_index,JSON_VALUE(@payload,'$.row_request_id') row_request_id,JSON_VALUE(@payload,'$.action') action,JSON_VALUE(@payload,'$.source_ref') source_ref,JSON_VALUE(@payload,'$.shopify_parent_ref') shopify_parent_ref,JSON_VALUE(@payload,'$.status') status,JSON_VALUE(@payload,'$.decision_event_id') decision_event_id,JSON_VALUE(@payload,'$.error') error,JSON_VALUE(@payload,'$.reviewed_by') reviewed_by,TIMESTAMP(JSON_VALUE(@payload,'$.recorded_at')) recorded_at) incoming ON target.event_id=incoming.event_id WHEN NOT MATCHED THEN INSERT ROW`,params:{payload:JSON.stringify(event)},types:{payload:'STRING'},useLegacySql:false});return event;});
  const summarizeOperation=rows=>{const latest=new Map();for(const row of rows)latest.set(Number(row.row_index),row);const results=[...latest.values()].sort((a,b)=>Number(a.row_index)-Number(b.row_index)).map(row=>({index:Number(row.row_index),request_id:row.row_request_id,action:row.action,source_ref:row.source_ref,status:row.status,event_id:row.decision_event_id||null,error:row.error||null}));const terminal=results.filter(x=>x.status!=='submitted');return{operation_id:rows[0]?.operation_id||null,status:results.length>0&&terminal.length===results.length?'complete':'processing',results,succeeded:terminal.filter(x=>['saved','already_applied'].includes(x.status)).length,blocked:terminal.filter(x=>x.status==='blocked').length,failed:terminal.filter(x=>x.status==='failed').length,pending:results.length-terminal.length};};
  ...
  return {
    ...
    async bulkReview({operation_id:operationId,decisions:requests=[]}={},reviewer){
      await setup();operationId=String(operationId||'');if(!operationId||operationId.length>128)throw new Error('operation_id is required for recoverable bulk review');if(!Array.isArray(requests)||!requests.length||requests.length>PRODUCT_MAPPING_MAX_BULK_SIZE)throw new Error(`bulk decisions must contain 1 to ${PRODUCT_MAPPING_MAX_BULK_SIZE} rows`);
      const existing=await operationEvents(operationId);if(existing.length){const known=new Set(existing.filter(x=>x.status==='submitted').map(x=>x.row_request_id));if(requests.some(x=>!known.has(String(x.request_id||''))))throw new Error('operation_id was already used for a different submission');}
      else for(let index=0;index<requests.length;index++){const request=requests[index]||{};await appendOperationEvent({operation_id:operationId,row_index:index,row_request_id:String(request.request_id||''),action:request.action||null,source_ref:request.source_ref||null,shopify_parent_ref:request.shopify_parent_ref||null,status:'submitted',decision_event_id:null,error:null,reviewed_by:reviewer});}
      const results=[];
      for(let index=0;index<requests.length;index++){
        const request=requests[index]||{},requestId=String(request.request_id||'');
        try{
          const prior=(await operationEvents(operationId)).find(x=>Number(x.row_index)===index&&x.status!=='submitted');if(prior){results.push({index,request_id:requestId,status:prior.status,event_id:prior.decision_event_id||null});continue;}
          ...
          if(already){const result={index,request_id:requestId,status:'already_applied',idempotent:true,event_id:eventId};await appendOperationEvent({operation_id:operationId,row_index:index,row_request_id:requestId,action:request.action,source_ref:request.source_ref,shopify_parent_ref:request.shopify_parent_ref||null,status:result.status,decision_event_id:eventId,error:null,reviewed_by:reviewer});results.push(result);continue;}
          if(request.action==='no_equivalent'||request.action==='needs_investigation'){
            const row={event_id:eventId,source_ref:source.source_product_ref,source_title:source.title||null,outcome:request.action,reviewed_by:reviewer,reviewed_at:new Date().toISOString(),provenance:'oracle_product_mapping_bulk_review',note:sanitizeReviewerNote(request.note),request_id:requestId};await appendOutcome(row);results.push({index,request_id:requestId,status:'saved',action:request.action,event_id:eventId});await appendOperationEvent({operation_id:operationId,row_index:index,row_request_id:requestId,action:request.action,source_ref:request.source_ref,shopify_parent_ref:null,status:'saved',decision_event_id:eventId,error:null,reviewed_by:reviewer});continue;
          }
          ...
          if(request.action==='identity'){
            ...
            await validateApproval(candidate,activeEdges(mappingHistory));const row=makeDecision(candidate,'approved',reviewer,{note:request.note,provenance:'oracle_product_mapping_bulk_review',requestId});await append([row]);results.push({index,request_id:requestId,status:'saved',action:'identity',event_id:row.event_id});
          }else{
            if(!preview.family_preview.allowed)throw new Error(`family blocked: ${preview.family_preview.reason}`);
            assertFamilyAssignment(source.source_product_ref,parent.source_product_ref,familyHistory);const row=makeFamilyDecision(source,parent,'active',reviewer,{note:request.note,provenance:'oracle_product_mapping_bulk_review',requestId});await appendFamily([row]);results.push({index,request_id:requestId,status:'saved',action:'family',event_id:row.event_id});
          }
          const result=results.at(-1);await appendOperationEvent({operation_id:operationId,row_index:index,row_request_id:requestId,action:request.action,source_ref:request.source_ref,shopify_parent_ref:request.shopify_parent_ref||null,status:result.status,decision_event_id:result.event_id,error:null,reviewed_by:reviewer});
        }catch(error){const status=/\bblocked\b|stale preview|already (?:an |)active/i.test(error.message)?'blocked':'failed';const result={index,request_id:requestId||null,status,action:request.action||null,error:String(error.message).slice(0,500)};await appendOperationEvent({operation_id:operationId,row_index:index,row_request_id:requestId||null,action:request.action||null,source_ref:request.source_ref||null,shopify_parent_ref:request.shopify_parent_ref||null,status,decision_event_id:null,error:result.error,reviewed_by:reviewer});results.push(result);}
      }
      return summarizeOperation(await operationEvents(operationId));
    },
    async bulkReviewStatus(operationId,reviewer){await setup();const rows=await operationEvents(String(operationId||''));if(!rows.length||rows.some(x=>x.reviewed_by!==reviewer))return null;const terminalIndexes=new Set(rows.filter(x=>x.status!=='submitted').map(x=>Number(x.row_index))),submitted=rows.filter(x=>x.status==='submitted'&&!terminalIndexes.has(Number(x.row_index)));if(submitted.length){const [mappingHistory,familyHistory,outcomeHistory]=await Promise.all([decisions(),familyDecisions(),reviewOutcomes()]),history=[...mappingHistory,...familyHistory,...outcomeHistory];for(const row of submitted){const decisionStatus=row.action==='identity'?'approved':row.action==='family'?'active':row.action,eventId=stableId(row.row_request_id,`${decisionStatus}:${row.source_ref}:${row.shopify_parent_ref||''}`);if(history.some(item=>(item.decision_id||item.event_id)===eventId))rows.push({...row,event_id:`reconciled:${eventId}`,status:'already_applied',decision_event_id:eventId,recorded_at:new Date().toISOString()});}}return summarizeOperation(rows);},
