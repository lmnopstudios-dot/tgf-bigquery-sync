...
export function buildHistoricalReviewQueue(products, candidates, { mappingDecisions=[], familyDecisions=[] } = {}) {
  ...
  return [...evidenceByRef].map(([ref,evidence])=>{const source=byRef.get(ref)||{source_product_ref:ref,source_platform:ref.split(':')[0],source_store:ref.split(':')[1],source_product_id:ref.split(':').slice(2).join(':')},identityDecision=identityByRef.get(ref),familyDecision=familyByRef.get(ref);return{historical_product:{...source,source_label:productSourceLabel(source)},resolution_status:identityDecision?'active_identity':familyDecision?'active_reporting_family':'needs_review',active_identity_mapping:identityDecision||null,active_family_assignment:familyDecision||null,evidence:evidence.sort((a,b)=>b.score-a.score),candidate:evidence[0]};}).sort((a,b)=>Number(b.historical_product.line_items||0)-Number(a.historical_product.line_items||0)||String(a.historical_product.title).localeCompare(String(b.historical_product.title)));
}
...
export function createProductMappingService({ bigquery, project, dataset = PRODUCT_MAPPING_DATASET }) {
  ...
  const buildList=async({products,search='',ensureSchema=true,diagnostic=false}={})=>{
    ...
    const [history,families]=await Promise.all([timed('mapping_decisions',decisions),timed('family_decisions',familyDecisions)]);
    ...
    let items=await timed('historical_grouping',()=>atGovernanceStage('historical_grouping',null,async()=>buildHistoricalReviewQueue(catalog,candidates,{mappingDecisions:history,familyDecisions:families})));
    if(search)items=items.filter(x=>`${x.historical_product.title} ${x.historical_product.source_product_id} ${x.historical_product.source_label}`.toLowerCase().includes(search.toLowerCase()));
    ...
    const counts={needs_review:items.length,active_identity:historical.filter(x=>identityRefs.has(x.source_product_ref)).length,active_reporting_family:historical.filter(x=>familyRefs.has(x.source_product_ref)).length};
    if(diagnostic)return{read_only:true,search_empty:search==='',stage:'complete',counts:{...counts,products:catalog.length,candidates:candidates.length,mapping_decisions:history.length,family_decisions:families.length},timing_ms:timings};
    return {items,counts,summary:`${items.length} historical products remain to review · ${identityRefs.size} have an active identity mapping · ${familyRefs.size} have an active reporting-family assignment.`};
  };
  ...
  const appendFamily=async rows=>atGovernanceStage('insert_decision','product_family_append',async()=>{const columns=PRODUCT_FAMILY_SCHEMA.map(x=>x.split(':')[0]);const projections=columns.map(name=>name==='reviewed_at'?`TIMESTAMP(JSON_VALUE(row,'$.${name}')) ${name}`:`JSON_VALUE(row,'$.${name}') ${name}`);await bigquery.query({query:`INSERT INTO \`${familyTable}\` (${columns.join(',')}) SELECT ${projections.join(',')} FROM UNNEST(JSON_QUERY_ARRAY(@payload)) row`,params:{payload:JSON.stringify(rows)},types:{payload:'STRING'},useLegacySql:false});return rows;});
  const makeFamilyDecision=(source,parent,status,reviewer,{note=null,provenance='oracle_product_family_review',supersedes=null,replacementFor=null}={})=>{const decision_id=crypto.randomUUID();return{event_id:decision_id,decision_id,membership_id:relationshipId(source.source_product_ref,parent.source_product_ref),source_ref:source.source_product_ref,source_title:source.title||null,shopify_parent_ref:parent.source_product_ref,shopify_parent_title:parent.title||null,status,reviewed_by:reviewer,reviewed_at:new Date().toISOString(),provenance,note:sanitizeReviewerNote(note),supersedes_decision_id:supersedes,replacement_for_decision_id:replacementFor};};
  ...
  return {
    setup, decisions, familyDecisions,
