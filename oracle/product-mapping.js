...
export function createProductMappingService({ bigquery, project, dataset = PRODUCT_MAPPING_DATASET }) {
  ...
  const validateApproval=async(candidate,edges)=>{const products=await loadProducts();return assertProductGraphIntegrity({products,explicitEdges:[...edges,{...candidate,mapping_status:'resolved'}],deterministicEdges:deterministicEdges(products)});};
  return {
    setup, decisions,
