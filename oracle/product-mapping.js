...
export function createProductMappingService({ bigquery, project, dataset = PRODUCT_MAPPING_DATASET }) {
  ...
  const decisions = async () => { const [rows] = await bigquery.query({ query:`SELECT * FROM \`${table}\` ORDER BY reviewed_at,COALESCE(decision_id,event_id)`, useLegacySql:false }); return rows.map(d=>({...d,decision_id:d.decision_id||d.event_id,relationship_id:d.relationship_id||relationshipId(d.left_ref,d.right_ref)})); };
  ...
  const append=async rows=>{const columns=fields.map(x=>x.split(':')[0]);const scalar=columns.filter(x=>x!=='candidate_evidence');const projections=scalar.map(name=>name==='score'?`SAFE_CAST(JSON_VALUE(row,'$.${name}') AS FLOAT64) ${name}`:name==='reviewed_at'?`TIMESTAMP(JSON_VALUE(row,'$.${name}')) ${name}`:`JSON_VALUE(row,'$.${name}') ${name}`);projections.splice(columns.indexOf('candidate_evidence'),0,"SAFE.PARSE_JSON(JSON_QUERY(row,'$.candidate_evidence')) candidate_evidence");await bigquery.query({query:`INSERT INTO \`${table}\` (${columns.join(',')}) SELECT ${projections.join(',')} FROM UNNEST(JSON_QUERY_ARRAY(@payload)) row`,params:{payload:JSON.stringify(rows)},types:{payload:'STRING'},useLegacySql:false});return rows;};
  ...
  return {
    ...
    async history(filters={}){await setup();const products=await loadProducts();const productByRef=new Map(products.map(p=>[p.source_product_ref,p]));const state=resolveMappingDecisions(await decisions());let items=state.history.map(d=>({...d,left_product:productByRef.get(d.left_ref)||null,right_product:productByRef.get(d.right_ref)||null,current:state.current.includes(d),canonical_relationship:d.status==='approved'&&!state.current.includes(d)?'historical':state.activeApproved.includes(d)?'active':'none'}));if(!filters.status)items=items.filter(d=>d.current);else if(filters.status!=='all')items=items.filter(d=>filters.status==='superseded'?state.superseded.includes(d):d.status===filters.status);if(filters.source)items=items.filter(d=>d.left_ref.startsWith(filters.source+':')||d.right_ref.startsWith(filters.source+':'));if(filters.search)items=items.filter(d=>`${d.left_title} ${d.right_title}`.toLowerCase().includes(String(filters.search).toLowerCase()));if(filters.reviewer)items=items.filter(d=>d.reviewed_by===filters.reviewer);if(filters.start_date)items=items.filter(d=>String(d.reviewed_at)>=filters.start_date);if(filters.end_date)items=items.filter(d=>String(d.reviewed_at)<=`${filters.end_date}T23:59:59.999Z`);return {items:items.sort((a,b)=>String(b.reviewed_at).localeCompare(String(a.reviewed_at))),counts:{approved:state.activeApproved.length,rejected:state.activeRejected.length,revoked:state.revoked.length,superseded:state.superseded.length}};},
    async review(candidate,status,reviewer,note=null){await setup();if(!['approved','rejected'].includes(status))throw new Error('status must be approved or rejected');const history=await decisions();if(status==='approved')validateGraphApproval(candidate,activeEdges(history));const [row]=await append([makeDecision(candidate,status,reviewer,{note})]);return row;},
    async chooseCorrect({candidate,selected,note},reviewer){await setup();assertPair({source_product_ref:candidate.left_ref,source_platform:candidate.left_ref.split(':')[0],source_store:candidate.left_ref.split(':')[1]},selected);const history=await decisions();const replacement={left_ref:candidate.left_ref,right_ref:selected.source_product_ref,left_title:candidate.left_title,right_title:selected.title,candidate_method:'human_replacement'};validateGraphApproval(replacement,activeEdges(history));const rejected=makeDecision(candidate,'rejected',reviewer,{note,provenance:'oracle_choose_correct_product'});const approved=makeDecision(replacement,'approved',reviewer,{note,provenance:'oracle_choose_correct_product',replacementFor:rejected.decision_id});await append([rejected,approved]);return {rejected,approved};},
    async createMapping({left,right,note},reviewer){await setup();assertPair(left,right);const pair={left_ref:left.source_product_ref,right_ref:right.source_product_ref,left_title:left.title,right_title:right.title,candidate_method:'human_created'};const history=await decisions();validateGraphApproval(pair,activeEdges(history));const [approved]=await append([makeDecision(pair,'approved',reviewer,{note,provenance:'oracle_manual_mapping'})]);return {approved};},
    async changeMapping({decision_id,replacement,note},reviewer){await setup();const history=await decisions(),state=resolveMappingDecisions(history),old=state.byId.get(decision_id);if(!old||!state.activeApproved.includes(old))throw new Error('approved mapping is not active');const anchored=old.left_ref===replacement.source_product_ref?old.right_ref:old.left_ref;const replacementPair={left_ref:anchored,right_ref:replacement.source_product_ref,left_title:old.left_ref===anchored?old.left_title:old.right_title,right_title:replacement.title,candidate_method:'human_replacement'};const withoutOld=state.activeApproved.filter(d=>d!==old);validateGraphApproval(replacementPair,approvedMappingEdges(withoutOld));const revoked=makeDecision(old,'superseded',reviewer,{note,provenance:'oracle_change_mapping',supersedes:decision_id});const approved=makeDecision(replacementPair,'approved',reviewer,{note,provenance:'oracle_change_mapping',replacementFor:decision_id});await append([revoked,approved]);return {superseded:revoked,approved};},
    ...
    async reconsider({decision_id,approve=false,note},reviewer){await setup();const state=resolveMappingDecisions(await decisions()),old=state.byId.get(decision_id);if(!old||old.status!=='rejected')throw new Error('rejected decision was not found');if(approve)validateGraphApproval(old,activeEdges(state.history));const [decision]=await append([makeDecision(old,approve?'approved':'reconsidered',reviewer,{note,provenance:'oracle_reconsider_rejection',supersedes:decision_id})]);return {decision};}
  };
}
