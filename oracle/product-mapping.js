...
export function buildProductMappingCoverage(products, queue, {mappingDecisions=[],familyDecisions=[],reviewOutcomes=[]}={}) {
  ...
  const familyRefs=new Set(resolveFamilyDecisions(familyDecisions).active.map(x=>x.source_ref));
  const outcomeByRef=new Map(reviewOutcomes.map(x=>[x.source_ref,x.outcome]));
  const historical=products.filter(x=>typeof x.source_product_ref==='string'&&!x.source_product_ref.startsWith('shopify:'));
  const summarize=rows=>({products:rows.length,order_lines:rows.reduce((n,x)=>n+Number(x.line_items||0),0),sales:rows.reduce((n,x)=>n+Number(x.sales||0),0)});
  const by_source={};for(const source of ['woo:ww','woo:usd','square:square']){const rows=historical.filter(x=>namespace(x)===source),unresolved=rows.filter(x=>!mappingRefs.has(x.source_product_ref)&&!familyRefs.has(x.source_product_ref));by_source[source]={source_label:productSourceLabel(source),...summarize(unresolved),identity_resolved:rows.filter(x=>mappingRefs.has(x.source_product_ref)).length,family_resolved:rows.filter(x=>familyRefs.has(x.source_product_ref)).length,no_equivalent:rows.filter(x=>outcomeByRef.get(x.source_product_ref)==='no_equivalent').length,needs_investigation:rows.filter(x=>outcomeByRef.get(x.source_product_ref)==='needs_investigation').length,classification_impact:Object.fromEntries([...new Set(unresolved.map(x=>classifyProduct(x.title,x)))].map(c=>[c,summarize(unresolved.filter(x=>classifyProduct(x.title,x)===c))]))};}
  return {by_source,completed:historical.filter(x=>mappingRefs.has(x.source_product_ref)||familyRefs.has(x.source_product_ref)||outcomeByRef.has(x.source_product_ref)).length,remaining:queue.filter(x=>x.resolution_status==='needs_review'||x.resolution_status==='needs_investigation').length,total:historical.length};
}
...
export function createProductMappingService({ bigquery, project, dataset = PRODUCT_MAPPING_DATASET }) {
  ...
  const buildList=async({products,search='',ensureSchema=true,diagnostic=false}={})=>{
    ...
    const candidates=await timed('candidate_generation',()=>atGovernanceStage('candidate_generation',null,async()=>generateMappingCandidates(catalog,{decisions:history,familyDecisions:families})));
    let items=await timed('historical_grouping',()=>atGovernanceStage('historical_grouping',null,async()=>buildHistoricalReviewQueue(catalog,candidates,{mappingDecisions:history,familyDecisions:families,reviewOutcomes:outcomes})));
    const allItems=items;items=items.filter(x=>!['active_identity','active_reporting_family'].includes(x.resolution_status));
    ...
    items=items.map(item=>{if(!item.proposed_shopify_parent)return item;try{return{...item,authoritative_preview:buildProductChoicePreview({products:catalog,mappingDecisions:history,familyDecisions:families,candidate:item.candidate,selectedRef:item.proposed_shopify_parent.source_product_ref})}}catch(error){return{...item,authoritative_preview:{identity_preview:{allowed:false,reason:error.message},family_preview:{allowed:false,reason:error.message}}}}});
    ...
    const counts={needs_review:allItems.filter(x=>x.resolution_status==='needs_review'||x.resolution_status==='needs_investigation').length,active_identity:historical.filter(x=>identityRefs.has(x.source_product_ref)).length,active_reporting_family:historical.filter(x=>familyRefs.has(x.source_product_ref)).length};
    ...
    if(diagnostic)return{read_only:true,search_empty:search==='',stage:'complete',counts:{...counts,products:catalog.length,candidates:candidates.length,mapping_decisions:history.length,family_decisions:families.length,review_outcomes:outcomes.length},coverage,timing_ms:timings};
    return {items,counts,coverage,summary:`${coverage.completed} completed · ${coverage.remaining} remaining · ${identityRefs.size} identity mappings · ${familyRefs.size} reporting-family assignments.`};
  };
