...
export function buildHistoricalReviewQueue(products, candidates, { mappingDecisions=[], familyDecisions=[] } = {}) {
  ...
  for(const candidate of candidates)for(const ref of [candidate.left_ref,candidate.right_ref]){if(ref.startsWith('shopify:'))continue;const evidence=evidenceByRef.get(ref)||[];evidence.push(candidate);evidenceByRef.set(ref,evidence);}
...
export function createProductMappingService({ bigquery, project, dataset = PRODUCT_MAPPING_DATASET }) {
  ...
  return {
    ...
    async list({ products, search = '' } = {}) { await setup(); const [history,families]=await Promise.all([decisions(),familyDecisions()]),catalog=products || await loadProducts(); const candidates=generateMappingCandidates(catalog,{decisions:history,familyDecisions:families});let items=buildHistoricalReviewQueue(catalog,candidates,{mappingDecisions:history,familyDecisions:families});if(search)items=items.filter(x=>`${x.historical_product.title} ${x.historical_product.source_product_id} ${x.historical_product.source_label}`.toLowerCase().includes(search.toLowerCase()));items=items.slice(0,250);const historical=catalog.filter(x=>!x.source_product_ref.startsWith('shopify:')),identityRefs=new Set(resolveMappingDecisions(history).activeApproved.flatMap(x=>[x.left_ref,x.right_ref]).filter(x=>!x.startsWith('shopify:'))),familyRefs=new Set(resolveFamilyDecisions(families).active.map(x=>x.source_ref));return {items,counts:{needs_review:items.length,active_identity:historical.filter(x=>identityRefs.has(x.source_product_ref)).length,active_reporting_family:historical.filter(x=>familyRefs.has(x.source_product_ref)).length},summary:`${items.length} historical products remain to review · ${identityRefs.size} have an active identity mapping · ${familyRefs.size} have an active reporting-family assignment.`}; },
