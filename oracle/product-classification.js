...
const PROVENANCE=['human_approved','business_governed','catalogue_metadata','canonical_propagated'];
...
export function deriveCatalogueClassifications(product,{now=new Date().toISOString()}={}){
  ...
  const add=(classification_type,classification_value,field,raw)=>{const row={subject_ref:ref,canonical_product_ref:product.canonical_product_ref||null,subject_grain:'source_product',classification_type,classification_value:token(classification_value),provenance:'catalogue_metadata',origin_subject_ref:ref,source_evidence:{field,raw_value:String(raw)},authority:'authoritative_catalogue',confidence:'high',status:'active',conflict_reason:null,effective_from:null,effective_to:null,supersedes_classification_id:null,recorded_at:now,recorded_by:'product-classification-sync',reviewed_at:null,reviewed_by:null};row.classification_id=id(row);out.push(row)};
  const metadata=product.metadata||{};
  for(const [field,rawValue] of Object.entries(metadata)){let decoded=rawValue;if(typeof rawValue==='string'&&/^(?:\[|\")/.test(rawValue)){try{decoded=JSON.parse(rawValue)}catch{}}for(const raw of (Array.isArray(decoded)?decoded:[decoded]).filter(Boolean)){
    const value=token(raw);
    if(PRODUCT_CATEGORIES.includes(value))add('product_category',value,field,raw);
    if(value==='collaboration'||value==='collab')add('product_group','collaboration',field,raw);
    if(value==='core')add('product_group','core',field,raw);
    const named=String(raw).match(/^collaboration\s*:\s*(.+)$/i);if(named&&token(named[1])){add('product_group','collaboration',field,raw);add('collaboration_name',token(named[1]),field,raw)}
  }}
...
export function materializeClassifications(products,{human=[],mappingEdges=[],now=new Date().toISOString()}={}){
  ...
  const precedence={human_approved:3,catalogue_metadata:2,canonical_propagated:1};const dedup=new Map();for(const row of rows){const key=`${row.subject_ref}|${row.classification_type}|${row.classification_value}`;if(!dedup.has(key)||precedence[row.provenance]>precedence[dedup.get(key).provenance])dedup.set(key,row)}return [...dedup.values()].map(validateProductClassification);
}
