...
const PROVENANCE=['human_approved','business_governed','catalogue_metadata','shopify_product_type','governed_shopify_product_type_mapping','shopify_governed_collection','woo_taxonomy','square_catalogue','canonical_propagated'];
...
export class ProductClassificationOperationError extends Error{
  constructor(operation,cause){super(`Product classification sync failed during ${operation}`,{cause});this.name='ProductClassificationOperationError';this.operation=operation}
}
export async function atProductClassificationOperation(operation,run){try{return await run()}catch(error){if(error instanceof ProductClassificationOperationError)throw error;throw new ProductClassificationOperationError(operation,error)}}
const PRODUCT_CLASSIFICATION_COLUMNS=['classification_id','subject_ref','canonical_product_ref','subject_grain','classification_type','classification_value','provenance','origin_subject_ref','source_evidence','authority','confidence','status','conflict_reason','effective_from','effective_to','supersedes_classification_id','recorded_at','recorded_by','reviewed_at','reviewed_by'];
export const productClassificationPayload=rows=>JSON.stringify(rows);
export function productClassificationMergeSql(table){const values=PRODUCT_CLASSIFICATION_COLUMNS.map(name=>name==='source_evidence'?`SAFE.PARSE_JSON(JSON_QUERY(row,'$.${name}')) ${name}`:['effective_from','effective_to'].includes(name)?`SAFE_CAST(JSON_VALUE(row,'$.${name}') AS DATE) ${name}`:['recorded_at','reviewed_at'].includes(name)?`SAFE_CAST(JSON_VALUE(row,'$.${name}') AS TIMESTAMP) ${name}`:`JSON_VALUE(row,'$.${name}') ${name}`);return `MERGE \`${table}\` t USING (SELECT ${values.join(',')} FROM UNNEST(JSON_QUERY_ARRAY(@payload)) row) s ON t.classification_id=s.classification_id WHEN MATCHED THEN UPDATE SET status=s.status,source_evidence=s.source_evidence,conflict_reason=s.conflict_reason WHEN NOT MATCHED THEN INSERT ROW`}
...
export function createProductClassificationService({bigquery,project,evidenceLoader,mappingLoader}){
  ...
  const query=(operation,options)=>atProductClassificationOperation(operation,()=>bigquery.query({...options,useLegacySql:false,labels:{component:'product_classification',operation}}));
  return {async setup(){await query('ensure_table',{query:productClassificationDdl(project)});},async sync(){await this.setup();const products=await atProductClassificationOperation('load_catalogue_evidence',evidenceLoader),edges=mappingLoader?await atProductClassificationOperation('load_governed_mappings',mappingLoader):[],desired=materializeClassifications(products,{mappingEdges:edges});const [existing]=await query('load_existing',{query:`SELECT classification_id,status,provenance FROM \`${table}\``});const byId=new Map(existing.map(x=>[x.classification_id,x]));if(desired.length)await query('merge_desired',{query:productClassificationMergeSql(table),params:{payload:productClassificationPayload(desired)},types:{payload:'STRING'}});const desiredIds=desired.map(x=>x.classification_id),retirable=existing.filter(x=>x.status==='active'&&['shopify_governed_collection','governed_shopify_product_type_mapping','shopify_product_type','canonical_propagated'].includes(x.provenance)&&!desiredIds.includes(x.classification_id));if(retirable.length)await query('retire_stale',{query:`UPDATE \`${table}\` SET status='superseded',effective_to=CURRENT_DATE() WHERE classification_id IN UNNEST(@ids) AND status='active'`,params:{ids:retirable.map(x=>x.classification_id)},types:{ids:['STRING']}});const counts=fn=>Object.fromEntries([...desired.reduce((m,row)=>{const key=fn(row);m.set(key,(m.get(key)||0)+1);return m},new Map())]);return {inserted:desired.filter(x=>!byId.has(x.classification_id)).length,updated:desired.filter(x=>byId.has(x.classification_id)&&byId.get(x.classification_id).status!==x.status).length,unchanged:desired.filter(x=>byId.has(x.classification_id)&&byId.get(x.classification_id).status===x.status).length,retired:retirable.length,conflicts:desired.filter(x=>x.status==='conflict').length,skipped_unknown:products.filter(p=>p.source_product_ref.startsWith('shopify:')&&!normalizeShopifyProductType(p.metadata?.product_type)).length,by_source:counts(row=>row.subject_ref.split(':',1)[0]),by_classification_type:counts(row=>row.classification_type),by_provenance:counts(row=>row.provenance)};}};
}
