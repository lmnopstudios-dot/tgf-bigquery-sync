...
export function createOracleUiRouter({ knowledgeService, bigquery, project, chat, generateProposals, reportService, productMappingService, collectionClassificationService, analysisJobStore, env = process.env, now = () => Date.now() }) {
  ...
  router.post('/product-mappings/bulk-review',authenticate,protectWrite,json,async(req,res)=>{try{if(!productMappingService)throw new Error('Product mappings are unavailable');const result=await productMappingService.bulkReview(req.body||{},req.oracleUser.sub);res.status(result.failed?207:201).json({success:result.failed===0,...result})}catch(error){console.error('Oracle product mapping bulk review failed:',mappingDiagnostic(req,'bulk_review',error));res.status(isGovernanceBusinessError(error)?409:503).json({success:false,error:governancePublicError(error)})}});
