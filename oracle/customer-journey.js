...
export const JOURNEY_GROUPS = Object.freeze(['summary','downstream_product','entry_product','collaboration_name','currency']);
export const FIRST_ORDER_SEMANTICS = Object.freeze(['first_observed_ever','first_observed_in_period']);
...
export function validateJourneyDefinition(input={}) {
  const allowed=new Set(['start_date','end_date','cohort_entry_start','cohort_entry_end','observation_end','first_order_semantic','cohort','subsequent','metrics','group_by','sort','limit']);
  for(const key of Object.keys(input))if(!allowed.has(key))throw new Error(`invalid journey field: ${key}`);
  const cohort_entry_start=input.cohort_entry_start||input.start_date,cohort_entry_end=input.cohort_entry_end||input.end_date,observation_end=input.observation_end||input.end_date;
  date(cohort_entry_start,'cohort_entry_start');date(cohort_entry_end,'cohort_entry_end');date(observation_end,'observation_end');if(cohort_entry_start>cohort_entry_end)throw new Error('cohort_entry_start must be on or before cohort_entry_end');if(cohort_entry_end>observation_end)throw new Error('observation_end must be on or after cohort_entry_end');
  const first_order_semantic=input.first_order_semantic||'first_observed_ever';if(!FIRST_ORDER_SEMANTICS.includes(first_order_semantic))throw new Error('invalid first_order_semantic');
  ...
  return {start_date:cohort_entry_start,end_date:cohort_entry_end,cohort_entry_start,cohort_entry_end,observation_end,first_order_semantic,cohort,subsequent,metrics,group_by,sort,limit};
}

export const JOURNEY_SEMANTICS=Object.freeze({
  date_window:'The cohort entry is inside its inclusive entry period. Later qualifying orders are measured through observation_end.',
  first_observed:'first_observed_ever sequences all available governed history before applying the cohort-entry dates; first_observed_in_period is the explicit window-relative alternative.',
  historical_coverage:'First observed ever means first observed in available governed history, not necessarily the customer\'s first-ever TGF purchase.',
...
export function aggregateJourneyEvents(events,{start_date,end_date,cohort_entry_start=start_date,cohort_entry_end=end_date,observation_end=end_date,first_order_semantic='first_observed_ever',entryClassification=null,minimumSequence=2,maximumSequence=null,withinDays=null}={}){
  const identified=events.filter(e=>e.customer_ref&&e.order_ref&&e.timestamp&&String(e.timestamp).slice(0,10)<=observation_end&&(first_order_semantic==='first_observed_ever'||String(e.timestamp).slice(0,10)>=cohort_entry_start));
  ...
  for(const [customer_ref,lines] of customers){const orders=new Map();for(const line of lines){const a=orders.get(line.order_ref)||[];a.push(line);orders.set(line.order_ref,a)}const ranked=[...orders.entries()].sort((a,b)=>String(a[1][0].timestamp).localeCompare(String(b[1][0].timestamp))||String(a[1][0].source||'').localeCompare(String(b[1][0].source||''))||String(a[0]).localeCompare(String(b[0])));const entry=ranked[0],entryDate=String(entry?.[1]?.[0]?.timestamp||'').slice(0,10);if(!entry||entryDate<cohort_entry_start||entryDate>cohort_entry_end||entryClassification&&!entry[1].some(x=>(x.classifications||[]).includes(entryClassification)))continue;cohort.push(customer_ref);for(let i=1;i<ranked.length;i++){const sequence=i+1,days=Math.floor((new Date(ranked[i][1][0].timestamp)-new Date(entry[1][0].timestamp))/86400000);if(sequence<minimumSequence||maximumSequence!==null&&sequence>maximumSequence||withinDays!==null&&days>withinDays)continue;for(const line of ranked[i][1])downstream.push({...line,customer_ref,order_sequence:sequence,days_after_entry:days})}}
...
export function customerJourneySql(project,groupBy='downstream_product') {
  ...
  const resultSql=groupBy==='collaboration_name'?`SELECT collaboration_name,COUNT(DISTINCT em.governed_customer_ref) entry_customers,COUNT(DISTINCT d.governed_customer_ref) repeat_customers,ROUND(100*SAFE_DIVIDE(COUNT(DISTINCT d.governed_customer_ref),COUNT(DISTINCT em.governed_customer_ref)),2) repeat_rate_percentage,COUNT(DISTINCT d.order_ref) downstream_orders FROM entry_matches em LEFT JOIN downstream_orders d USING(governed_customer_ref) WHERE collaboration_name IS NOT NULL GROUP BY collaboration_name ORDER BY repeat_rate_percentage DESC,entry_customers DESC`
    :groupBy==='entry_product'?`SELECT ROW_NUMBER() OVER(ORDER BY repeat_rate_percentage DESC,cohort_customers DESC,entry_product) rank,* FROM (
    ...
    :`SELECT ROW_NUMBER() OVER(ORDER BY p.returning_customers DESC,p.downstream_orders DESC,p.product,p.source_product_ref) rank,p.product,p.returning_customers product_returning_customers,p.downstream_orders,p.units,p.net_sales_by_currency,ROUND(100*SAFE_DIVIDE(p.returning_customers,s.returning_customers),2) returning_cohort_penetration_percentage,s.cohort_customers,s.returning_customers cohort_returning_customers,ROUND(100*SAFE_DIVIDE(s.returning_customers,s.cohort_customers),2) repeat_rate_percentage FROM products p CROSS JOIN cohort_summary s ORDER BY p.returning_customers DESC,p.downstream_orders DESC,p.product LIMIT @limit`;
  ...
  orders AS (
    ...
    FROM orders WHERE source_customer_id IS NOT NULL AND source_customer_id NOT IN ('','0') AND NOT migrated
      ...
      AND COALESCE(order_total,0)>COALESCE(refunds,0) AND DATE(order_timestamp)<=@observation_end AND (@first_order_semantic='first_observed_ever' OR DATE(order_timestamp)>=@cohort_entry_start)
  ), lines AS (
  ...
  entry_matches AS (SELECT DISTINCT e.governed_customer_ref,e.order_ref entry_order_ref,e.order_timestamp entry_timestamp,l.source_product_ref entry_product_ref,l.product_title entry_product_title,(SELECT REPLACE(x,'collaboration_name=','') FROM UNNEST(l.classifications) x WHERE STARTS_WITH(x,'collaboration_name=') LIMIT 1) collaboration_name
    ...
    WHERE (@entry_product_ref IS NULL OR l.source_product_ref=@entry_product_ref) AND (@entry_classification IS NULL OR CONCAT('product_group=',@entry_classification) IN UNNEST(l.classifications) OR CONCAT('product_category=',@entry_classification) IN UNNEST(l.classifications) OR CONCAT('product_type=',@entry_classification) IN UNNEST(l.classifications))),
