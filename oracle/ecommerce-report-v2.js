...
export function createEcommerceReportV2({bigquery,project,knowledgeService}){
  ...
  ), pairs AS (SELECT 'woo_ww' left_ns,'shopify' right_ns UNION ALL SELECT 'woo_usd','shopify' UNION ALL SELECT 'woo_ww','woo_usd' UNION ALL SELECT 'square','shopify'), candidates AS (
  ...
  ), unique_edges AS (SELECT * FROM candidates QUALIFY COUNT(*) OVER(PARTITION BY left_ns,left_id,mapping_method)=1 AND COUNT(*) OVER(PARTITION BY right_ns,right_id,mapping_method)=1 AND ROW_NUMBER() OVER(PARTITION BY left_ns,left_id,right_ns,right_id ORDER BY IF(mapping_method='exact_unique_sku',0,1))=1), refs AS (
    SELECT source_namespace,source_product_id,MIN(peer_ref) canonical_product_ref,ARRAY_AGG(DISTINCT mapping_method ORDER BY mapping_method LIMIT 1)[OFFSET(0)] mapping_method FROM (
  ...
  ), classified AS (SELECT l.*,p.base_title,p.normalized_base_title,COALESCE(CONCAT('canonical:',r.canonical_product_ref),CONCAT('source:',l.source_namespace,':',COALESCE(l.source_product_id,'unknown'))) product_ref,COALESCE(r.mapping_method,'source_identity_only') mapping_method,IF(r.canonical_product_ref IS NULL,'source_specific','resolved') mapping_status FROM lines l JOIN base_products p USING(source_namespace,source_platform,source_store,source_product_id) LEFT JOIN refs r USING(source_namespace,source_product_id))
  SELECT product_ref,ANY_VALUE(base_title) canonical_title,ANY_VALUE(line_title) source_title,ANY_VALUE(normalized_base_title) normalized_title,ANY_VALUE(sku) source_sku,mapping_method,mapping_status,source_platform,source_store,channel,currency,source_product_id,ARRAY_AGG(DISTINCT source_variant_id IGNORE NULLS LIMIT 100) source_variant_ids,COUNT(DISTINCT source_variant_id) variant_count,SUM(units) units,SUM(product_sales) product_sales,COUNT(*) line_items,'stable_source_product_with_child_variants' provenance,'product' report_grain
