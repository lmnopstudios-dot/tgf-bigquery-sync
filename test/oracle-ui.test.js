...
test('lost eight-row bulk response is recoverable and retry cannot duplicate decisions',async t=>{
  const operations=new Map(),decisions=new Set();let postCalls=0;
  const service={
    async bulkReview(input){postCalls++;const started=Date.now(),rows=[];for(const [index,item] of input.decisions.entries()){const duplicate=decisions.has(item.request_id);decisions.add(item.request_id);rows.push({row_index:index,row_request_id:item.request_id,source_ref:item.source_ref,status:duplicate?'already_applied':'saved',decision_event_id:`decision-${index}`});await new Promise(resolve=>setTimeout(resolve,4));}const result={operation_id:input.operation_id,status:'complete',results:rows.map(row=>({index:row.row_index,request_id:row.row_request_id,source_ref:row.source_ref,status:row.status,event_id:row.decision_event_id})),succeeded:8,blocked:0,failed:0,pending:0,duration_ms:Date.now()-started};operations.set(input.operation_id,result);await new Promise(resolve=>setTimeout(resolve,80));return result;},
    async bulkReviewStatus(id){return operations.get(id)||null;}
  };
  const app=express();app.use('/api/oracle',createOracleUiRouter({knowledgeService:{},bigquery:{},project:'test',chat:async()=>({answer:''}),productMappingService:service,env}));const server=await new Promise(resolve=>{const value=app.listen(0,()=>resolve(value))});t.after(()=>server.close());const base=`http://127.0.0.1:${server.address().port}/api/oracle`,request=(path,options={})=>fetch(base+path,options),auth=await login(request,base),headers={cookie:auth.cookie,origin:new URL(base).origin,'content-type':'application/json','x-csrf-token':auth.csrf};
  const operation_id='production-shaped-eight',payload={operation_id,decisions:Array.from({length:8},(_,index)=>({request_id:`row-${index}`,action:'identity',source_ref:`woo:ww:${index}`,shopify_parent_ref:`shopify:shopify:${index}`}))};
  const controller=new AbortController();setTimeout(()=>controller.abort(),55);await assert.rejects(request('/product-mappings/bulk-review',{method:'POST',headers,body:JSON.stringify(payload),signal:controller.signal}),/abort/i);
  await new Promise(resolve=>setTimeout(resolve,80));const recovered=await request(`/product-mappings/bulk-review/${operation_id}`,{headers:{cookie:auth.cookie}}),body=await recovered.json();assert.equal(recovered.status,200);assert.equal(body.status,'complete');assert.equal(body.results.length,8);assert.ok(body.results.every(row=>row.status==='saved'));assert.ok(body.duration_ms<250,'eight rows must finish in a bounded production-shaped interval');
  const retry=await request('/product-mappings/bulk-review',{method:'POST',headers,body:JSON.stringify(payload)}),retried=await retry.json();assert.equal(retry.status,201);assert.equal(retried.succeeded,8);assert.equal(decisions.size,8);assert.equal(postCalls,2);
});

test('bulk mapping UI persists a stable operation and recovers authoritative per-row outcomes',async()=>{const source=await readFile(new URL('../public/oracle/app.js',import.meta.url),'utf8');assert.match(source,/bulkOperationKey='oracle-product-mapping-bulk-operation'/);assert.match(source,/localStorage\.setItem\(bulkOperationKey/);assert.match(source,/bulk-review\/\$\{encodeURIComponent\(operation\.operation_id\)\}/);assert.match(source,/saved\/already applied/);assert.match(source,/selected\.length>8/)});
...
test('explicit timeless-definition save request reaches chat and proposal generation without date scope',async t=>{
  const message='Please propose and save a timeless operational definition for made-to-order availability.';
  const calls=[],proposalCalls=[];
  const knowledgeService={searchKnowledge:async()=>({items:[]}),searchMemory:async()=>({items:[]})};
  const proposal={kind:'definition',term:'Made-to-order availability',definition:'Zero stock is made to order only for tagged products.',implementation_reference:'Operational policy',effective_from:null,effective_to:null,status:'confirmed',source_type:'human_entered',source_reference:'Authenticated administrator assertion',tags:['made-to-order'],supersedes:null};
  const app=express();app.use('/api/oracle',createOracleUiRouter({knowledgeService,bigquery:{},project:'test',chat:async(value,conversation)=>{calls.push({value,conversation});return {answer:'I prepared the timeless definition for review and Save.',tools:[]}},generateProposals:async input=>{proposalCalls.push(input);return [proposal]},env,now:()=>Date.parse('2026-09-25T12:00:00Z')}));
  const server=await new Promise(resolve=>{const value=app.listen(0,()=>resolve(value))});t.after(()=>server.close());
  const base=`http://127.0.0.1:${server.address().port}/api/oracle`,request=(path,options={})=>fetch(base+path,options),auth=await login(request,base),headers={cookie:auth.cookie,origin:new URL(base).origin,'content-type':'application/json','x-csrf-token':auth.csrf};
  const response=await request('/chat',{method:'POST',headers,body:JSON.stringify({message})}),body=await response.json();
  assert.equal(response.status,200);assert.equal(calls.length,1);assert.equal(calls[0].value,message);
