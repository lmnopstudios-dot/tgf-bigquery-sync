...
export async function collect({ client, propertyId, startDate, endDate, maxRows = 100000, syncedAt = new Date().toISOString() }) {
  ...
  for (const r of rows(reports[4], ['date', 'eventName'], ['eventCount'])) { const date = normalizeGa4Date(r.date); if (!events.has(date)) events.set(date, {}); events.get(date)[r.eventName] = r.eventCount; }
  ...
  return validateCollected({ daily, acquisition, landing_pages, device_geo, ecommerce_funnel }, startDate, endDate);
}
export function validateCollected(data, startDate, endDate) {
  ...
  for (const name of TABLES) { const seen = new Set(); for (const row of data[name]) { const vals = Object.values(row); if (vals.some(v => typeof v === 'number' && (!Number.isFinite(v) || v < 0))) throw new Error(`${name} contains invalid negative/non-finite metric`); if ('engagement_rate' in row && row.engagement_rate > 1) throw new Error(`${name} engagement_rate exceeds 1`); const key = name === 'daily' || name === 'ecommerce_funnel' ? row.date : JSON.stringify(Object.entries(row).filter(([k]) => k === 'date' || !['sessions','total_users','engaged_sessions','engagement_rate','screen_page_views','synced_at'].includes(k))); if (seen.has(key)) throw new Error(`${name} contains duplicate dimensional key`); seen.add(key); } }
